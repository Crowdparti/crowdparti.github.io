...
(function(){ return function (fin, math, gltf, ge, html) {
              ...
              on_mouse_drage: function (dx, dy, e) {
                  ...
                  if (mouse_drage) {
                     ...
                     // app.root.transform.set_eular(app.root.eular[0], app.root.eular[1], app.root.eular[2]);
                      ...
                      mouse_drage(dx, dy, app.root.eular);
          ...
          function setup_system(renderables) {
              ...
              pas.extend(function (sys) {
                  ...
                  sys.system_working = false;

                  sys.commands[512] = function (cmd, ci) {
                      sys.system_working = cmd[ci++] === 1;
                  };

                  var em = sys.define_emitter(1000, function (proto, _super) {
                      ...
                      proto.create_instance = proto.override(proto.create_instance)(function (_super_call) {
                          ...
                          return function (ins, params, pi) {
                              ...
                              ins.type = params[pi + 3];

                              ins.x = params[pi + 4];
                              ins.y = params[pi + 5];
                              ins.z = params[pi + 6];

                              v3[0] = params[pi + 7] - ins.x;
                              v3[1] = params[pi + 8] - ins.y;
                              v3[2] = params[pi + 9] - ins.z;

                              S0$x = v3[0] * v3[0] + v3[1] * v3[1] + v3[2] * v3[2];
      if (S0$x > 0) {
        S0$x = 1 / Math.sqrt(S0$x);
      }
      v3[0] = v3[0] * S0$x; v3[1] = v3[1] * S0$x; v3[2] = v3[2] * S0$x;
                      ...
                      proto.trigger_instance = function (ins, sys) {
                          ...
                          if (ins.type === 1) {
                              if (sys.system_working) {
                                  for (i = 0; i < 20; i++) {
                                      par = sys.queue_particle(ins.emi.get_particle(ins), (i / 30) * 2, ins.id, ins.par_life);
                      ...
                      proto.create_particle = function (par, ins) {
                          ...
                          par[4] = ins.y;
                          par[5] = ins.z;
                          par[6] = ins.dx + (Math.random() - 0.5) * 0.65;
                      ...
                      var sucks = [{
                          ...
                          z: 0,
                          pow: 12,
                          check: function (p, dist) {
                              p[4] = Math.max(p[4], this.y);

                              if (p[3] > this.x) p[3] = this.x;
                              return p[4] < -3 && dist < 9.5;
                          }
                      },
                      {
                          x: -9.5,
                      ...
                      proto.update_particle = function (par, ins, time_delta) {
                          ...
                          if (ins.type === 1) {
                              if (sys.system_working)
                                 ...
                                 // par[7] = ins.dy + (Math.random() - 0.5) * 3.5;
                          ...
                          for (si = 0; si < sucks.length; si++) {
                              ...
                              if (suc.type >1) {
                                  ...
                                  dd = pdir[0] * pdir[0] + pdir[1] * pdir[1];
                              }
                              pdist = Math.sqrt(dd);
      ...
      if (uni) {
        ...
        uni.func.apply(shader.gl, uni.params);
                          ...
                          uni = shader.uniforms["u_par_size"];
      if (uni) {
        uni.params[uni.params_length] = u_par_size;
        uni.func.apply(shader.gl, uni.params);
      }

                          uni = shader.uniforms["u_par_colors[0]"];
      if (uni) {
        uni.params[uni.params_length] = u_par_colors[0];
        uni.func.apply(shader.gl, uni.params);
      }
                          uni = shader.uniforms["u_par_colors[1]"];
      if (uni) {
        uni.params[uni.params_length] = u_par_colors[1];
        uni.func.apply(shader.gl, uni.params);
      }
                          uni = shader.uniforms["u_par_colors[2]"];
      if (uni) {
        uni.params[uni.params_length] = u_par_colors[2];
        uni.func.apply(shader.gl, uni.params);
      }
                        ...
                        //   renderer.gl.blendFunc(770, 771);
                          ...
                          renderer.gl.drawArrays(0, 0, mesh.draw_count);
...
uniform mat4 u_view_projection_rw;
uniform mat4 u_model_rw;
uniform vec4 u_par_colors[3];
uniform vec3 u_par_size;

varying vec4 v_par_color;
...
int i=int(a_position_rw.w);
gl_PointSize =(u_par_size[i]/gl_Position.w);


v_par_color=u_par_colors[i];
...
gl_FragColor.xyz*=v_par_color.xyz;
gl_FragColor.w*=v_par_color.w;
}
...
gl_FragColor.rgb=vec3(1.0)-gl_FragColor.rgb;
              ...
              pas.output(11000, function (dt, oi, count) {
                  if (!ppos.buffer) return;
...
`);
             ...
             // var uvmesh= setup_uv();
              ...
              pas.worker.commands_group(function (self, cmd, worker) {
                  ...
                  app.set_system_working = function (s) {
                      app.system_working = s;
                  ...
                  app.set_system_working(app.system_working);
              ...
              var btnSystem = html.elm$('<div class="button duct_is_on_button"></div>');
              document.body.appendChild(btnSystem);
              btnSystem.onclick = function () {

                  if (app.set_system_working(!app.system_working)) {
                      btnSystem.classList.add("duct_is_off_button");
                     // btn.innerHTML = "DUCT SYSTEM ENABLED";
                  }
                  else {
                      btnSystem.classList.remove("duct_is_off_button");
              ...
              function ar_features() {
                  ...
                  u_par_size = math.vec3(50.0, 70.0, 90.0);
                     ...
                     // app.root.transform.set_scale(0.13, 0.13, 0.13);
                      fin.timers.start(function () {
                          ...
                          if (markResult) {
                            ...
                            //  console.log(markResult.idMatrix + "," + markResult.idPatt)
                            ...
                            //  app.root.transform.set_rot(qu1[0], qu1[1], qu1[2], qu1[3]);
          ...
          fin.urls_loader([["duct-system.obj"], ["duct-system.mtl"], ["human.gltf"]], function (datas) {
              ...
              console.log(human);
              var human_geo = ge.geometry.geometry_data.create(human.geos[0]);

              human_geo.scale_position_rotation(2.5, 2.5, 2.5, 0, 0, 0, 0, 0, 0);
